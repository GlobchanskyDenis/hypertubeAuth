 ...
 */
func createRepairTokenSignature(header string) (string, *errors.Error) {
	conf, Err := getConfig()
	if Err != nil {
		return "", Err
	}
	header += conf.PasswdSalt + "passwordRepair"
 ...
 */
func checkRepairTokenPartsSignature(header, origSignature string) *errors.Error {
	signature, Err := createRepairTokenSignature(header)
	if Err != nil {
		return Err
	}
