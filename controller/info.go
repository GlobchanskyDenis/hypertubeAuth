...
func info(w http.ResponseWriter, r *http.Request) {
	...
	В случае провала или отказа пользователя предоставлять права произойдет редирект НА ТВОЙ эндпоинт c ошибкой в json
	и дополнительно завернутой в base64 в параметре url. Примеры редиректов (после рефакторинга эндпоинты могут измениться):</br>
	<b>http://localhost:3000/auth?accessToken=token</b></br>
	<b>http://localhost:3000/login?error=base64encodedJsonWithError</b></br>
	...
	В случае провала страница после редиректа должна считать параметр url, декодировать его из base64, затем из json и вывести сообщение на экран (ru, en)</br></br>
	...
	<b>PATCH /api/email/patch - изменение почты пользователя</b></br>
	Описание не готово</br></br>
